      ...
      window.addEventListener('DOMContentLoaded', function() {
        ...
        function makeUserFriendlyStack(stack, src) {
          ...
          for (var i = 1; i < lines.length - 1; i++) {
            ...
            // match pattern of
            //   at eval (eval at <anonymous> (chrome-extension://hneablcelipadealmebgomejjmllfphg/cm-local/cm-eval-sandbox.html:23:43), <anonymous>:6:3)
            //  (developer only cares about the end)
            line = line.replace(/^(\s*at\s+[^\s]+).*(<anonymous>:.+)[)]?$/, '$1 $2');
            newStack += ('\n' + line);
          }
          // skip the last line,  in the pattern of 
          //   at chrome-extension://hneablcelipadealmebgomejjmllfphg/cm-local/cm-eval-sandbox.html:23:28"
